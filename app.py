...
# 1. 페이지 설정
st.set_page_config(page_title="부동산 v85 UI/Bug Fix", layout="centered")
...
st.markdown("""
    ...
    /* 전화번호: 12px로 축소 및 라벨(TEL)과의 간격 15px 확보 */
...
with tab1:
    st.markdown("""
    <div style="background-color: #f9f9f9; padding: 10px; border-radius: 10px; border: 1px solid #ddd; margin-bottom: 10px; font-size: 11px;">
        <b>🎨 마커 색상 기준 (12.5억 대비)</b> | <span style="color:red;">●</span> 1.5억 초과 | 
        <span style="color:blue;">●</span> 1.5억 이하 | <span style="color:orange;">★</span> 1억 하락(급매)
    </div>
    """, unsafe_allow_html=True)
    ...
    # [수정] 변수명 map_data를 명확히 정의하여 아래 if문에서 참조 가능하게 함
...
with tab2:
    ...
    if mode == "단지등록":
        ...
        if h_type == "기존 매물 업데이트" and not apt_hoga_df.empty:
            ...
            sel_item = st.selectbox("추적 매물", item_opts)
            ...
            st.info(f"📍 기존 정보 로드됨 | 이전가: {prev_val}억")
...
with tab3:
    ...
    if df_key in st.session_state:
        edited_df = st.data_editor(st.session_state[df_key], use_container_width=True, num_rows="dynamic", key=f"ed_{target}")
        if st.button("💾 시트 일괄 저장"): save_cloud_data(edited_df, target)import streamlit as st
import pandas as pd
import folium
from streamlit_folium import st_folium
from streamlit_gsheets import GSheetsConnection
...
# 1. 페이지 설정
st.set_page_config(page_title="부동산 v84 UI Final", layout="centered")
...
st.markdown("""
    ...
    /* [수정] 전화번호 폰트 12px로 축소 및 라벨과의 간격 15px 확보 */
    .phone-link { color: #007AFF !important; text-decoration: none; font-size: 12px !important; margin-left: 15px; font-weight: 500; display: inline-block; }
    .phone-row { display: flex; align-items: center; margin-bottom: 4px; min-height: 22px; }
    .phone-label { color: #999; width: 38px; font-size: 10px !important; font-weight: bold; text-align: left; }
    </style>
...
with tab1:
    ...
    for _, row in st.session_state.complex_df.iterrows():
        if pd.notnull(row['위도']) and row['위도'] != 0:
            ...
            # [수정] 가격 16px 유지, 변동액(+0.00) 부분을 13px로 확대
            if not h_df.empty:
                min_h = h_df.loc[h_df['현재호가(억)'].idxmin()]
                h_val, h_diff = float(min_h['현재호가(억)']), float(min_h['호가변동'])
                if h_diff <= -1.0: color, icon = "orange", "star"
...
with tab3:
    target = st.selectbox("데이터 시트", ["apart", "real", "hoga"])
    edited_df = st.data_editor(st.session_state[f"{target}_df" if "df" not in target else target], use_container_width=True, num_rows="dynamic", key=f"ed_{target}")
    if st.button("💾 시트 일괄 저장"): save_cloud_data(edited_df, target)

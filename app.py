...
# 1. 페이지 설정
st.set_page_config(page_title="부동산 v80 Tracking", layout="centered")
...
def save_cloud_data(df, ws_name):
    try:
        ...
        st.success(f"✅ {ws_name} 시트 업데이트 완료!")
        st.cache_data.clear()
        st.rerun()
    except Exception as e:
        st.error(f"❌ 저장 실패: {e}")

# --- UI 스타일링 (전화번호 11px 고정) ---
st.markdown("""
    <style>
    ...
    for _, row in st.session_state.complex_df.iterrows():
        if pd.notnull(row['위도']) and row['위도'] != 0:
            ...
            <div style='width: 200px; font-family: sans-serif;'>
                ...
                <div style='margin-bottom: 10px;'>{tel_html}</div>
...
with tab2:
    ...
    with st.form("input_v80"):
        if mode == "단지등록":
            ...
            h_type = st.radio("방식 선택", ["단지별 매물 추적(기존)", "완전 신규 매물 등록"], horizontal=True)
            f_apt = st.selectbox("아파트 단지", st.session_state.complex_df['아파트명'].unique())
            ...
            if h_type == "단지별 매물 추적(기존)":
                if not apt_hoga_df.empty:
                    ...
                    st.info(f"선택: {f_dong}동 {f_floor}층 | 이전가: {prev_val}억")
                else:
                    st.warning("이 단지엔 등록된 매물이 없습니다. 신규 등록을 이용하세요.")
            else:
                c1, c2, c3 = st.columns(3)
                f_dong, f_floor, f_size = c1.text_input("동"), c2.text_input("층"), c3.text_input("평형")

            f_hoga = st.number_input("신규 호가(억)", format="%.2f")
            if st.form_submit_button("📢 호가 데이터 누적 저장"):
...
# --- 탭 3: 데이터 관리 ---
with tab3:
    target = st.selectbox("시트", ["apart", "real", "hoga"])
